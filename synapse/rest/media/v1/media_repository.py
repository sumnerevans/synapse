...
class MediaRepository:
    ...
    async def create_media_id(self, auth_user: UserID) -> Tuple[str, int]:
        ...
        unused_expires_at = now + (60 * 1000)
        await self.store.store_local_media_id(
            media_id=media_id,
            time_now_ms=now,
            user_id=auth_user,
            # After 60 seconds, don't allow the upload to start.
    ...
    async def get_remote_media(
        ...
        responder, media_info = await self._get_remote_media_impl_with_retries(
    ...
    async def _get_remote_media_impl_with_retries(
        self, server_name: str, media_id: str, max_stall_ms: int
        ...
        while True:
            ...
            # remote server.
            await self.clock.sleep(retry_delay_seconds)
